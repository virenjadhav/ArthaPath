class BudgetsController < ApplicationController
    def get_budgets
        result, message, @budgets  = BudgetCrud.get_budgets(@doc)
        if result
            object = {data: @budgets, message: "Budgets Reload successfully.", status: "success"}
            respond_to_action(object)
        else 
            msg = message.blank? ? "Something went wrong getting budgets." : message
            respond_to_error(msg)
        end
    end
  
    def show_budget
        result, message, @budget = BudgetCrud.show_budget(@doc)
        if result
            object = {data: @budget, message: "Budget get successfully", status: "success"}
            respond_to_action(object)
        else 
            msg = message.blank? ? @budget.errors : message
            respond_to_error(msg)
        end
    end
    def create_or_save_budget      
        create_record = @doc[:id].blank? ? true : false
        result, message, @budget = BudgetCrud.create_or_save_budget(budget_params)
        if result
            object = {data: @budget, message: "Budget was successfully #{create_record ? "created" : "updated"}.", status: "success"}
            respond_to_action(object)
        else 
            msg = message.blank? ? @budget.errors : message
            respond_to_error(msg)
    ...
    def destroy_budget     
        result, message, @budget = BudgetCrud.delete_budget(@doc)
        if result
            object = {data: @budget, message: "Budget deleted successfully", status: "success"}
            respond_to_action(object)
        else 
            msg = message.blank? ? @budget.errors : message
            respond_to_error(msg)
    ...
    private
    def budget_params
      params.require(:body).permit(:id, :active, :amount, :main_category_id, :main_category_code, :sub_category_id, :sub_category_code,  :description, :user_id,  :payment_method, :account_id, :account_code, :tag, :period, :from_date, :to_date, :alert_amount)
    end
